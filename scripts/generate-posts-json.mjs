    ...
    for (const file of mdxFiles) {
      ...
      try {
        ...
        if (!data.title || !data.date || !data.summary) {
          console.warn(`🟡 WARN: Skipping ${file} - Missing required frontmatter (title, date, or summary).`);
          continue; // Skip this file
        }

        // Create the URL slug from the filename
        const slug = file.replace(/\.mdx$/, '');
        const url = `<span class="math-inline">\{SITE\_POSTS\_BASE\_URL\}</span>{slug}.html`;
        ...
        postsData.push({
          ...
          date: data.date.toISOString ? data.date.toISOString().split('T')[0] : String(data.date), 
          url: url,
        });
      } catch (parseError) {
        console.error(`🔴 ERROR: Failed to parse frontmatter for ${file}.`, parseError);
